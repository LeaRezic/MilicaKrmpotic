...
namespace RWAWebForms
{
    public partial class UsersList : MyPage
    {
        ...
        {
            if (!(Session["CurrentUser"] as User).IsAdmin)
            {
                mojGV.Columns[5].Visible = false;
                ClientScript.RegisterStartupScript(GetType(), "youShallNotPass", "nonAdminYouShallNotPass()", true);
            }
        }
        ...
        protected void myGridView_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                int IDUser = (int)mojGV.DataKeys[e.Row.RowIndex].Value;
        ...
        protected void myRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                LinkButton editButton = (LinkButton)e.Item.FindControl("lbtnEdit");
                editButton.PostBackUrl = "~/EditOneUser.aspx?UserID=" + urlID;
                if (!(Session["CurrentUser"] as User).IsAdmin)
                {
